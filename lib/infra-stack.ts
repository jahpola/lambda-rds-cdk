...
export class LambdaRdsStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const db = new rds.ServerlessCluster(this, 'db', {
      ...
      // default ie no subnetgroup = private_with_egress automatically selected
    ...
    const reader = new NodejsFunction(this, 'reader-function', {
      ...
      environment: {
        ...
        // unsafeUnwrap is used to get the value from the secret, not recommended in production
        DBNAME: db.secret?.secretValueFromJson('dbname').unsafeUnwrap()!,
        USER: db.secret?.secretValueFromJson('username').unsafeUnwrap()!,
        SECRET: db.secret?.secretValueFromJson('password').unsafeUnwrap()!
      },
      vpc: vpc,
      vpcSubnets: {
        // PRIVATE_WITH_EGRESS is required with outside access usually
        // ISOLATION might require private link+api etc endpoints, 
        // recommendation: just use private_with_egress unless working in high security environments
